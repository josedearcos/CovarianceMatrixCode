...
const bool WriteOutput=1;//To save the covariance matrices in a .txt file.
...
public:
    CovarianceMatrix1();
    CovarianceMatrix1(NominalData Data);
...
CovarianceMatrix1 :: CovarianceMatrix1()
{
    Nom = new NominalData();
    rand = new TRandom3();
    Pred = new Prediction();
...
CovarianceMatrix1 :: CovarianceMatrix1(NominalData Data)
{
    rand = new TRandom3();
    ...
    cout<<InitialBins<<"IS INITIAL BINS"<<endl;
...
void CovarianceMatrix1 :: CovarianceMatrixMain()
{
    LoadBackgrounds();
    LoadPredictions();
    LoadNearHall();
...
//    TestGaussianH->SetDirectory(0);
    ...
    CovarianceMatrix1* NLObject = new CovarianceMatrix1();//Finally the standard constructor is useful
    switch (NLModelE)
    {
        case 0://BCW NL Model
    ...
    for (Int_t samples = 0; samples<NSamples; samples++)
    {
        ...
        switch (NLModelE)
        {
            case 0://BCW NL Model
                ...
                nl_func = new TF1("nl_func",NLObject,&CovarianceMatrix1::nl_func_bcw,InitialVisibleEnergy,FinalVisibleEnergy,7,"CovarianceMatrix1","nl_func_bcw");
            ...
            case 1://LBNL NL Model
                ...
                nl_func = new TF1("nl_func",NLObject,&CovarianceMatrix1::nl_func_lbnl,InitialVisibleEnergy,FinalVisibleEnergy,5,"CovarianceMatrix1","nl_func_lbnl");
                for (Int_t idet = 0; idet < ADsEH3; idet++)
                {
        ...
        CovarianceMatrix1* ResoObject = new CovarianceMatrix1();//Finally the standard constructor is useful
        
        reso_func = new TF1("reso_func",ResoObject,&CovarianceMatrix1::reso_func_bcw,0,20,3,"CovarianceMatrix1","reso_func_bcw");
        ...
        delete reso_func;
        delete VisibleF;
        ResoObject->~CovarianceMatrix1();
        NLObject->~CovarianceMatrix1();
...
void CovarianceMatrix1 :: GenerateCovarianceMatrix()
{    
...
//Add detector effects //Multiply Matrix and NearHallSpectrum
            for(Int_t i=0;i<TotalBins;i++)
            {
                ...
                {
                    Sum = Sum+(EnergyMatrixH[near]->GetBinContent(i+1,j+1)*NearHallSpectrumH[near][week]->GetBinContent(i+1));
//                    cout << "SUM " << Sum << endl;
//                        cout << " i: " << i <<  "; j: " << j << endl;
                }
...
//                RandomPredictionH1[far][near][week]->SetDirectory(0);
                ...
                for(Int_t i=0;i<TotalBins;i++)
                {
                    ...
                    for(Int_t j=0; j<TotalBins; j++)
                    {
                          Sum = Sum+(EnergyMatrixH[far]->GetBinContent(i+1,j+1)*PredictionH[far][near][week]->GetBinContent(i+1));
//                        cout << "SUM " << Sum << endl;
//                        cout << " i: " << i <<  "; j: " << j << endl;
                    }
                    if (Sum<0)
...
//                    RandomPredictionH1[far][near][week]->Add(RandomAmCH[ADsEH1+ADsEH2+far][week]);
    ...
    TH2F* ABkgCov2H = new TH2F("Background Cov Matrix","Background Covariance Matrix",TotalBins*9,0,TotalBins*9,TotalBins*9,0,TotalBins*9);
...
//                        cout <<"\n";
                        ...
                        {//columns
                            ...
                            for (Int_t j = 0; j<TotalBins; j++)
                            {//rows
                                x = i+(Ni1*Fi1+Ni2*Fi2+Ni3*Fi3+Ni4*Fi4-1)*TotalBins;
                                y = j+(Nj1*Fj1+Nj2*Fj2+Nj3*Fj3+Nj4*Fj4-1)*TotalBins;
...
////////////////////////////////////////////////////////////////////////////////////
void CovarianceMatrix1 :: GenerateStatisticalCovarianceMatrix()
{
    for (Int_t week = 0; week<Nweeks; week++)
    {
        //Add nominal backgrounds
        ...
        for (Int_t far=0; far<ADsEH3; far++)
        {
            for (Int_t near=0; near<(ADsEH1+ADsEH2); near++)
            {
                for (Int_t pts = 0; pts < TotalBins; pts++)
                {
    ...
    TH2F* AStatCov2H = new TH2F("Statistical Cov Matrix","Statistical Covariance Matrix",TotalBins*9,0,TotalBins*9,TotalBins*9,0,TotalBins*9);
...
void CovarianceMatrix1 :: RandomEnergyScaleMatrix()
{
    
}
void CovarianceMatrix1 :: RandomIAVMatrix()
{
    for (Int_t AD=0; AD<NADs; AD++)
    {
...
void CovarianceMatrix1 :: RandomNLMatrix()
{
    switch (NLModelE)
    {
        case 0://BCW NL Model
...
void CovarianceMatrix1 :: RandomResolutionMatrix()
{
    
}
void CovarianceMatrix1 :: CreateEnergyMatrix()
{
    TFile* SaveSpectrumDataF = TFile::Open("./RootOutputs/EnergyMatrix1.root","recreate");
    ...
    for(int idet=0; idet<ADsEH3; idet++)
    {
        ...
        EnergyMatrixH[idet]->SetDirectory(0);
...
void CovarianceMatrix1 :: GetEnergyShift(Int_t TrueEnergyIndex)
{
    ...
    for(int idet=0; idet<ADsEH3; idet++)
    {
        ...
        if (PositronEnergy < 1.015||TrueEnergyIndex>=MaxPositronEnergyIndex)//1.8-0.78 is about the minimum visible energy (1.017999 for 0th order, 1.01574 for 1st order calculation)
        {
...
void CovarianceMatrix1 :: GetIAVShift(Int_t TrueEnergyIndex)
{
    for(int idet=0; idet<ADsEH3; idet++)
    {
        //IAV
        ...
        //Calculate IAV Shift
        if (PositronEnergy >= 1.015)//1.8-0.78 is about the minimum visible energy (1.017999 for 0th order, 1.01574 for 1st order calculation)
        {
            //Reset values to 0
            for(Int_t VisibleEnergyIndex=0; VisibleEnergyIndex<TotalBins; VisibleEnergyIndex++)
            {
...
void CovarianceMatrix1 :: GetNLShift(Int_t TrueEnergyIndex)
{
    for(int idet=0; idet<ADsEH3; idet++)
    {
        //NL
        ...
        for(Int_t VisibleEnergyIndex=0; VisibleEnergyIndex<TotalBins; VisibleEnergyIndex++)
        {
            PositronNLSpectrumH[idet][TrueEnergyIndex]->SetBinContent(VisibleEnergyIndex+1, 0);
        }
        
        for(Int_t VisibleEnergyIndex=0; VisibleEnergyIndex<TotalBins; VisibleEnergyIndex++)
        {
            PositronNLSpectrumH[idet][TrueEnergyIndex]->SetBinContent(floor(nl_func->Eval(VisibleEnergyIndex)+1+0.5),PositronIAVSpectrumH[idet][TrueEnergyIndex]->GetBinContent(VisibleEnergyIndex+1));
             // cout << "positron NL index" << floor(nl_func->Eval(VisibleEnergyIndex)+1+0.5) << "with values" << PositronIAVSpectrumH[idet][TrueEnergyIndex]->GetBinContent(VisibleEnergyIndex+1) << "and true energy" << TrueEnergyIndex << endl;
            //weird behaviour at TrueEnergy = 66; 117 and 167 due to  nl_func->Eval(VisibleEnergyIndex)+1. I need to round to the closest integer.
        }
    }
}

void CovarianceMatrix1 :: GetResolutionShift(Int_t TrueEnergyIndex)
{
    for(Int_t idet=0; idet<ADsEH3; idet++)
    {        
        //Resolution
        ...
        for(Int_t VisibleEnergyIndex=0; VisibleEnergyIndex<TotalBins; VisibleEnergyIndex++)
        {
            PositronVisibleSpectrumH[idet][TrueEnergyIndex]->SetBinContent(VisibleEnergyIndex+1, 0);//Reset values to 0
        }

        //Calculate Resolution effect
        
        for(Int_t VisibleEnergyIndex=0; VisibleEnergyIndex<TotalBins+1; VisibleEnergyIndex++)
        {
            ...
            Norm[TrueEnergyIndex] = Norm[TrueEnergyIndex] + PositronVisibleSpectrumH[idet][TrueEnergyIndex]->GetBinContent(VisibleEnergyIndex+1);
        }
    }
}

void CovarianceMatrix1 :: NormalizeEnergyMatrix(Int_t TrueEnergyIndex)
{
    for(Int_t idet=0; idet<ADsEH3; idet++)
    {
        ...
        EnergySlice[idet][TrueEnergyIndex]->SetDirectory(0);
        if(Norm[TrueEnergyIndex]!=0)
        {
            EnergySlice[idet][TrueEnergyIndex]->Scale(1./Norm[TrueEnergyIndex]);
        }
    }
}
...
void CovarianceMatrix1 :: LoadNLParameters()
{
    switch (NLModelE)
    {
        case 0://BCW NL Model
...
void CovarianceMatrix1 :: LoadBackgrounds()
{
...
void CovarianceMatrix1 :: FluctuateBackgrounds()
{
    for (Int_t week = 0; week<Nweeks; week++)
    {
            for (Int_t AD =0; AD<NADs; AD++)
...
void CovarianceMatrix1 :: LoadPredictions()
{
    ...
    for (Int_t week = 0; week<Nweeks; week++)
    {
        for (Int_t near =0; near<(ADsEH1+ADsEH2); near++)
        {
            for (Int_t far =0; far<ADsEH3; far++)
            {
                ...
                OriginalPredictionH[far][near][week]->SetDirectory(0);
                
                PredictionH[far][near][week] = new TH1F(Form("Padded AD%i Far Spectrum prediction from near AD%i",far+1,near+1),Form(" Padded AD%i Far Spectrum prediction from near AD%i",far+1,near+1),TotalBins,InitialVisibleEnergy, FinalVisibleEnergy);
                PredictionH[far][near][week]->SetDirectory(0);
                
                for(Int_t i = 1; i<=InitialBins;i++)
                {
                    PredictionH[far][near][week]->SetBinContent(i,0);//Padding
                }
                for(Int_t i = 1; i<=(Nbins+InitialBins);i++)
                {
                    PredictionH[far][near][week]->SetBinContent(i+InitialBins,OriginalPredictionH[far][near][week]->GetBinContent(i));
                }
            }
        }
    }
...
void CovarianceMatrix1 :: LoadNearHall()
{
    ...
    for (Int_t week = 0; week<Nweeks; week++)
    {
        for (Int_t near =0; near<(ADsEH1+ADsEH2); near++)
        {
            ...
            OriginalNearHallSpectrumH[near][week]->SetDirectory(0);
    
            NearHallSpectrumH[near][week] = new TH1F(Form("Padded Total spectrum after oscillation at AD%i",near+1),Form("Padded Total spectrum after oscillation at AD%i",near+1),TotalBins,InitialVisibleEnergy, FinalVisibleEnergy);
            NearHallSpectrumH[near][week]->SetDirectory(0);
            for(Int_t i = 1; i<=InitialBins;i++)
            {
                 NearHallSpectrumH[near][week]->SetBinContent(i,0);//Padding
            }
            for(Int_t i = 1; i<=(Nbins+InitialBins);i++)
            {
                NearHallSpectrumH[near][week]->SetBinContent(i+InitialBins,OriginalNearHallSpectrumH[near][week]->GetBinContent(i));
            }
        }
    }
    NearHallDataF->Close();
...
void CovarianceMatrix1 :: LoadIavCorrection()//From Bryce Littlejohn's results. //I don't use different IAV matrix for each AD since the analysis it's been done for just 1 of them, assume identical ADs.
{
...
void CovarianceMatrix1 :: SaveTotalSpectrum()
{
    for (Int_t week = 0; week<Nweeks; week++)
    {
        for (Int_t near =0; near<(ADsEH1+ADsEH2); near++)
...
void CovarianceMatrix1 :: SaveCovarianceMatrix()
{
...
void CovarianceMatrix1 :: SetAccidentalMatrix(bool AccMatrix)
{
...
void CovarianceMatrix1 :: SetLiHeMatrix(bool LiMatrix)
{
    LiHeMatrix=LiMatrix;
    if(LiMatrix)
    {
...
void CovarianceMatrix1 :: SetFastNeutronsMatrix(bool FNmatrix)
{
...
void CovarianceMatrix1 :: SetAmCMatrix(bool AmCmatrix)
{
    AmCMatrix=AmCmatrix;
    if(AmCMatrix)
    {
...
void CovarianceMatrix1 :: SetIsotopeMatrix(bool IsotopeMatrix)
{
    if(IsotopeMatrix)
    {
...
void CovarianceMatrix1 :: SetReactorPowerMatrix(bool ReactorPowerMatrix)
{
...
void CovarianceMatrix1 :: SetEnergyScaleMatrix(bool EnergyScaleMatrix)
{
...
void CovarianceMatrix1 :: SetIAVMatrix(bool IAVMatrix)
{
    if(IAVMatrix)
    {
        RandomIAVMatrix();
...
void CovarianceMatrix1 :: SetNLMatrix(bool NLMatrix)
{
    if(NLMatrix)
    {
        RandomNLMatrix();
...
void CovarianceMatrix1 :: SetResolutionMatrix(bool ResolutionMatrix)
{
    if(ResolutionMatrix)
    {
        RandomResolutionMatrix();
...
void CovarianceMatrix1 :: SetBCWModel(bool bcw)
{
    if(bcw)
    {
        NLModelE=BCWE;
...
void CovarianceMatrix1 :: SetLBNLModel(bool lbnl)
{
    if(lbnl)
    {
        NLModelE=LBNLE;
...
void CovarianceMatrix1 :: SetIHEPModel(bool ihep)
{
    if(ihep)
    {
        NLModelE=IHEPE;
...
TF1* CovarianceMatrix1 :: GetNeutrinoToVisibleFunction(Int_t order)
{
    if(order==0)
    {
        Double_t Correction = Mn-Mp-Me;
...
TF1* CovarianceMatrix1 :: GetDistortionFunction(Double_t amount)
{
...
TF1* CovarianceMatrix1 :: GetFastNeutronsDistortionFunction(Double_t amount)
{
...
Double_t CovarianceMatrix1 :: reso_func_bcw(Double_t* x, Double_t* par)
{
    Double_t e_orig = x[0];
...
Double_t CovarianceMatrix1 :: nl_func_bcw(Double_t* x, Double_t* par)
{
    //Input error
...
Double_t CovarianceMatrix1 :: nl_func_lbnl(Double_t * x, Double_t * par)
{
