...
void LoadEnergyM(){
    ...
    Data->SetAnalysis(1);//Gd or H data
    Data->SetEmin(1.8);
    Data->SetEmax(12.0);;
    if(Data->GetAnalysis())//Hydrogen data
    {
        Data->SetWeeks(1); //1 or 20.
        Data->SetNbins(51);//41,51 or 820 (Check 241 when I use IAV model)
    }
    else//Gd data
    {
        Data->SetWeeks(1); //1 or 20.
    ...
    for (Int_t samples = 0; samples<NSamples; samples++)
    {
        for (Int_t week = 0; week<Nweeks; week++)
        {
            ...
            if(DistortBackgrounds)
            {
                if(AccidentalMatrix)
                {                    
                    TF1 *func_acc = new TF1("func","TMath::Abs([0]+[1]*x)",InitialEnergy,FinalEnergy);
                    rand->SetSeed(0);
                    Double_t slope=DistortAcc*rand->Gaus(0,1);
        ...
        for (Int_t week = 0; week<Nweeks; week++)
        {
            ...
            for (Int_t near=0; near<1; near++)//(ADsEH1+ADsEH2)
            {
                cout << " NEAR " << near << endl;
        ...
        for (Int_t week = 0; week<Nweeks; week++)
        {
            for (Int_t neari=0; neari<(1); neari++)
            {
                //Logic for the 2D matrix index done up to 8 ADs
                if(neari==0){Int_t Ni1=1;Int_t Ni2=0;Int_t Ni3=0;Int_t Ni4=0;}
                if(neari==1){Ni2++;}
                ...
                for (Int_t fari=0; fari<1; fari++)
                {
                    ...
                    for (Int_t nearj=0; nearj<(1); nearj++)
                    {
                        //Logic for the 2D matrix index done up to 8 ADs
                        if(nearj==0){Int_t Nj1=1;Int_t Nj2=0;Int_t Nj3=0;Int_t Nj4=0;}
                        if(nearj==1){Nj2++;}
