...
const bool FlatEnergyMatrix = 1;// This one shouldn't make any change since the matrix is normalized. DayaBay asked to use a flat spectrum to produce the EvisEnu matrix.
...
void CreateEnergyMatrix :: SetBCWModel(bool bcw)
{
    ...
    {
        NLModelE=BCWE;
        NLF = new TF1("NLF",this,&CreateEnergyMatrix::NLBCWF,InitialVisibleEnergy,FinalVisibleEnergy,7,"Prediction","NLBCWF");
    }
}
...
void CreateEnergyMatrix :: SetLBNLModel(bool lbnl)
{
    ...
    {
        NLModelE=LBNLE;
        NLF = new TF1("NLF",this,&CreateEnergyMatrix::NLLBNLF,InitialVisibleEnergy,FinalVisibleEnergy,5,"Prediction","NLLBNLF");
    }
}
...
void CreateEnergyMatrix :: SetUnifiedModel(bool unified)
{
    ...
    {
        NLModelE=UnifiedE;
        NLF = new TF1("NLF",this,&CreateEnergyMatrix::NLUnifiedF,InitialVisibleEnergy,FinalVisibleEnergy,m_num_unified_nl_pars+2,"Prediction","NLUnifiedF");
    }
}

TF1* CreateEnergyMatrix :: GetNeutrinoToVisibleFunction(bool order)
{
    if(order==0)
    {
        Double_t Correction = Mn-Mp-Me;
        VisibleF = new TF1("VisibleF",this,&CreateEnergyMatrix::VisibleEnergy0F,InitialEnergy,FinalVisibleEnergy,1,"Prediction","VisibleEnergy0F");
        VisibleF->SetParameter(0,Correction);
        std::cout<<"Zeroth order"<<std::endl;
    }
    if(order==1)
    {
        VisibleF = new TF1("VisibleF",this,&CreateEnergyMatrix::VisibleEnergy1F,InitialEnergy,FinalVisibleEnergy,1,"Prediction","VisibleEnergy1F");
...
void CreateEnergyMatrix :: Interpolation(TF1* func)
{
    ...
    {
        Energy[VisibleEnergyIndex]=(func->GetX(VisibleEnergyIndex*BinWidth));//GetX
...
void CreateEnergyMatrix :: GetOscEnergyShift(Int_t TrueEnergyIndex, Int_t week)
{
    ...
    if(FlatEnergyMatrix)
    {
        OscDeltaPositronSpectrumH[TrueEnergyIndex]->SetBinContent(TrueEnergyIndex-Int_t(1.016/BinWidth)+1,1);
    }
//    else
//    {
...
void CreateEnergyMatrix :: GetOscIAVShift(Int_t TrueEnergyIndex, Int_t week)
{
    ...
    //Calculate IAV Shift
    if (PositronEnergy >= 1.016)//2*0.511
    {
        for(Int_t VisibleEnergyIndex=0; VisibleEnergyIndex<TrueEnergyIndex+1; VisibleEnergyIndex++)
        {
                OscDeltaIAVSpectrumH[TrueEnergyIndex]->SetBinContent(VisibleEnergyIndex+1, OscDeltaIAVSpectrumH[TrueEnergyIndex]->GetBinContent(VisibleEnergyIndex+1) + IAVMatrix[0][TrueEnergyIndex-Int_t(1.016/BinWidth)][VisibleEnergyIndex] * OscDeltaPositronSpectrumH[TrueEnergyIndex]->GetBinContent(TrueEnergyIndex-Int_t(1.016/BinWidth)+1));
        }
    }
}
